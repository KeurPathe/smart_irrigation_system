...
def create_scene():
    ...
    box(size=vector(length_box, height_box, box_thick/2),
        ...
        color=vector(0, 0.9, 0), opacity=0.95)

    # lines
    lines_opacity = 0.1
    lines_color = vector(0.01, 0.01, 0.01)
    i = 0
    while (i < height_box/2):
        line1 = box(size=vector(length_box, box_thick, deep_box/4),
                    pos=vector(0, i, 0),
                    color=lines_color, opacity=lines_opacity)
        i = i + 0.5
    i = 0
    while (i > -height_box/2):
        box(size=vector(length_box, box_thick, deep_box/3),
            pos=vector(0, i, 0),
            color=lines_color, opacity=lines_opacity)
        i = i - 0.5

    i = 0
    while (i < length_box/2):
        box(size=vector(box_thick, height_box, deep_box/3),
            pos=vector(i, 0, 0),
            color=lines_color, opacity=lines_opacity)
        i = i + 0.5
    i = 0
    ...
    box(size=vector(length_box, box_thick, deep_box/3),
        pos=vector(0, height_box/2 - height_box/3, 0),
        color=lines_color, opacity=0.2)
    box(size=vector(length_box, box_thick, deep_box/3),
        pos=vector(0, -height_box/2 + height_box/3, 0),
        color=lines_color, opacity=0.2)
    box(size=vector(box_thick, height_box, deep_box/3),
